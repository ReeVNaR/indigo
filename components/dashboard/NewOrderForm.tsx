...
export default function NewOrderForm({ isOpen, onOpenChange, onOrderCreated, customers }: NewOrderFormProps) {
    ...
    const toggleGarment = (type: 'Shirt' | 'Pant' | 'Kurta') => {
        ...
        if (idx !== -1) {
            ...
            setNewOrderForm(p => ({
                ...p,
                items: [...p.items, { garment_type: type, quantity: 1, price: 0, fabric_source: 'Customer' }]
            }));
        }
    };
    ...
    const renderStep2 = () => (
        <div className="space-y-6">
            ...
            {newOrderForm.items.length > 0 && (
                <div className="bg-[#131c3f] rounded-xl p-4 flex justify-between items-center shadow-lg shadow-[#131c3f]/20 animate-in fade-in duration-300">
                    <span className="text-xs font-black text-amber-400/80 uppercase tracking-widest">Total Estimated</span>
                ...
                <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    ...
                    {currentStep < 3 ? (
                        <button
                            ...
                            className="bg-[#131c3f] hover:bg-[#0f1626] text-white px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md"
                        >
